...
@click.option("--languages", type=str, help="Languages available in the contest 'c11-clang'|'c11-gcc'|'cat'|'cpp11-clang'|'cpp11-gcc'|'cpp17-clang'|'cpp17-gcc'|'cpp20-clang'|'cpp20-gcc'|'cs'|'go'|'hs'|'java'|'js'|'kj'|'kp'|'kt'|'lua'|'pas'|'py2'|'py3'|'rb'|'rs'|null")
@click.option("--scoreboard", type=int, help="Integer representing the porcentage of time the scoreboard will be visible")
async def create(ctx, contest_title, contest_start, contest_duration, custom_id, description, languages, scoreboard):
    """Create contest test.\n 
    ...
    creation = OmegaUpApi.create_contest(contest_id, contest_title, contest_start, contest_duration, description, languages, scoreboard)
    click.echo(creation)
